...
class BraitenbergSystem {
	...
	*/
	...
	constructor(p) {
		...
		// (or write to!)
		this.map = new MapBuffer({p, mapEraseRate: 0.001,  mapScale: 4, blur: true})
	...
	update(p, time) {
		...
		// this.map.drawIntoBuffer((mapBuffer) => {
		// 	// Each lamp draws at a different size
		// 	this.vehicles.forEach((v,lampIndex) => v.drawBlurryCircle({
		// 		p: mapBuffer,
		// 		layers: 2,
		// 		radius: 100,
		// 		// opacity: .7 + .04*Math.sin(lampIndex + 2*time.t),
...
class BraitenbergVehicle extends Vector {
	...
	update(p, time) {
		...
		let spinRate = .02 + .2*sliders.vehicleTurn
		this.forwardAngle += (this.wheels[0] - this.wheels[1])*spinRate

		// How much to accelerate given the wheel's speeds
		let driveForce = (500)*(.2 + 2*sliders.vehicleSpeed)*(this.wheels[0] + this.wheels[1])
		...
		// Apply lots of drag
		this.velocity.mult(1 - sliders.vehicleDrag*1)
	...
	draw(p) {
		...
		p.fill(0)
		p.stroke(100)
		p.ellipse(-this.bodySize*.8, 0, this.bodySize*2, this.bodySize*1.2)
		p.circle(this.bodySize*.1, 0, this.bodySize)

		// Draw the wheels
		let wheelLength = this.bodySize*.8
		let wheelWidth = this.bodySize*.3
		
		this.wheels.forEach((w,wIndex) => 
				p.rect(-wheelLength*1.4, 
				6*wheelWidth*(wIndex-.5) - wheelWidth/2, 
				wheelLength, wheelWidth))
		
		this.eyes.forEach((eyeVal,eyeIndex) => {
			let r = this.eyeRadius
			let theta = this.eyeAngle*2*(eyeIndex - .5)
			let eyeSize = this.bodySize*.7
			p.fill(eyeVal*100)
			p.circle(r*Math.cos(theta), r*Math.sin(theta), eyeSize)
		})

		// Draw their cutie mark
		let marks = ["♥️", "⚔️", "⭐️", "💀"]
		p.rotate(Math.PI/2)
		p.textSize(this.bodySize)
		p.text(marks[this.type], -this.bodySize/2, this.bodySize*1.4)
