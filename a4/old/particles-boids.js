...
class Boid {
	...
	update(p) {
		...
		this.forces.separation.mult(SLIDER.separation)
		...
		this.forces.cohesion.mult(-2*SLIDER.cohesion)
		...
		this.forces.alignment.mult(500*SLIDER.alignment)
		this.totalForce.add(this.forces.alignment)

		
		// Thrust
		this.forces.thrust.setToPolar(140*SLIDER.thrust, this.angle)
		this.totalForce.add(this.forces.thrust)
		
		this.forces.loopiness.setToPolar(500*(p.noise(this.id*2, t*.1) - .5), this.angle + Math.PI/2)
		this.totalForce.add(this.forces.loopiness)
		
		// Wander
		this.forces.wander.setToPolar(0*p.noise(this.id, t*.2), 20*p.noise(this.id, t*.14))
		this.totalForce.add(this.forces.wander)
	...
	draw(p) {
		...
		this.forces.loopiness.drawArrow({p, 
				multiple:1,
